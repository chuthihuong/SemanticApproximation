...
public class MainPP extends Class_Random {
	public MainPP() {}
...
//			}
			// Compute Complexity at generation 0
			 ...
			 complexity=0;
			 for(int k=0; k<NUMVAR; k++){
				 t1= (myPop.bestcurrent[0].semanticTraning[IndexUniquefeature[1][k]]- 
						 myPop.bestcurrent[0].semanticTraning[IndexUniquefeature[0][k]]) 
						 /(myPop.fitcase[IndexUniquefeature[1][k]].x[k]-myPop.fitcase[IndexUniquefeature[0][k]].x[k]);
				
				 for(int j=1;j<NumUniquefeature[k];j++){
					 t2= (myPop.bestcurrent[0].semanticTraning[IndexUniquefeature[j+1][k]]- 
							 myPop.bestcurrent[0].semanticTraning[IndexUniquefeature[j][k]]) 
...
//				f1.print(myPop.genotypeDist[i]+";");
				...
				complexity=0;
				 for(int k=0; k<NUMVAR; k++){
					 t1= (myPop.bestcurrent[i].semanticTraning[IndexUniquefeature[1][k]]- 
							 myPop.bestcurrent[i].semanticTraning[IndexUniquefeature[0][k]]) 
							 /(myPop.fitcase[IndexUniquefeature[1][k]].x[k]-myPop.fitcase[IndexUniquefeature[0][k]].x[k]);
					
					 for(int j=1;j<NumUniquefeature[k];j++){
						 t2= (myPop.bestcurrent[i].semanticTraning[IndexUniquefeature[j+1][k]]- 
								 myPop.bestcurrent[i].semanticTraning[IndexUniquefeature[j][k]]) 
								 /(myPop.fitcase[IndexUniquefeature[j+1][k]].x[k]-myPop.fitcase[IndexUniquefeature[j][k]].x[k]);
						 complexity+=Math.abs(t2-t1);
						 t1=t2;
					 }
